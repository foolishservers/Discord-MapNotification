...
public void OnPluginStart()
{
	...
	Cvar[WebhookName] = AutoExecConfig_CreateConVar("discord_map_notification_webhook", "MapNotification", "Discord webhook name for this plugin (addons/sourcemod/configs/DMN_Discord.cfg)");
	Cvar[Avatar] = AutoExecConfig_CreateConVar("discord_map_notification_avatar", "https://csgottt.com/map_notification.png", "URL to Avatar image");
	Cvar[Username] = AutoExecConfig_CreateConVar("discord_map_notification_username", "Map Notifications", "Discord username");
	Cvar[Color] = AutoExecConfig_CreateConVar("discord_map_notification_colors", "16738740", "Decimal color code\nHex to Decimal - https://www.rapidtables.com/convert/number/hex-to-decimal.html");
	Cvar[LangCode] = AutoExecConfig_CreateConVar("discord_map_notification_language_code", "en", "Which language (as 2 or 3 digit code) for discord messages?\nHere's a list of some/all languages codes:\nhttps://en.wikipedia.org/wiki/List_of_ISO_639-1_codes");
	Cvar[CurrentGame] = AutoExecConfig_CreateConVar("discord_map_notification_game", "csgo", "Which game directory for images? (Default: csgo)");
	Cvar[Logo] = AutoExecConfig_CreateConVar("discord_custom_logo_url", "", "If you want to set a custom logo for the embedded discord message, fill this with your logo url out.\nIf you use custom logo, map picture (from gametracker) will be ignored.");
	Cvar[Icon] = AutoExecConfig_CreateConVar("discord_map_notification_icon", "https://csgottt.com/map_notification.png", "URL for footer icon (empty for disabling this feature)");
	Cvar[Timestamp] = AutoExecConfig_CreateConVar("discord_map_notification_timestamp", "1", "Show timestamp/date in footer? (0 - Disabled, 1 - Enabled)", _, true, 0.0, true, 1.0);
	Cvar[Title] = AutoExecConfig_CreateConVar("discord_map_notification_title", "Custom title", "Set a custom title text or leave it blank for showing the hostname");
	Cvar[FooterText] = AutoExecConfig_CreateConVar("discord_map_notification_footer", "Here's the custom footer text.", "Set a custom footer text or leave it blank for showing the hostname");
	Cvar[RedirectURL] = AutoExecConfig_CreateConVar("discord_map_notification_redirect", "https://server.bara.dev/redirect.php", "URL to your redirect.php file, you can also use my redirect.php which is located in germany.");
	Cvar[ServerIP] = AutoExecConfig_CreateConVar("discord_map_notification_server_ip", "", "Set custom server ip or hostname. Keep it empty if you don't want it.");
	Cvar[ServerPort] = AutoExecConfig_CreateConVar("discord_map_notification_server_port", "0", "Set custom server port. Keep it 0 if you don't want it.", _, true, 0.0);
...
void PrepareAndSendMessage(bool test)
{
	...
	char sIP[32];
	Cvar[ServerIP].GetString(sIP, sizeof(sIP));
	if (!sIP[0])
	{
		int ips[4];
	...
	int iPort = Cvar[ServerPort].IntValue;
	if (iPort < 1)
	{
		iPort = cvar.IntValue;
	}
	...
	char sConnect[512];
	char sURL[256];
	Cvar[RedirectURL].GetString(sURL, sizeof(sURL));
	Format(sConnect, sizeof(sConnect), "[%s:%d](%s?ip=%s&port=%d)", sIP, iPort, sURL, sIP, iPort);

	char sGame[18];
	Cvar[CurrentGame].GetString(sGame, sizeof(sGame));

	/* Set bot avatar */
	char sThumb[256];
	Cvar[Logo].GetString(sThumb, sizeof(sThumb));
	if (!sThumb[0])
	{
	...
	Cvar[WebhookName].GetString(sWeb, sizeof(sWeb));
	...
	char sName[128];
	Cvar[Username].GetString(sName, sizeof(sName));
	wWebhook.SetUsername(sName);

	char sAvatar[256];
	Cvar[Avatar].GetString(sAvatar, sizeof(sAvatar));
	wWebhook.SetAvatarURL(sAvatar);

	char sCode[4];
	Cvar[LangCode].GetString(sCode, sizeof(sCode));

	int iLang = GetLanguageByCode(sCode);

	Embed eEmbed = new Embed();
	eEmbed.SetColor(Cvar[Color].IntValue);

	char sTitle[512];
	Cvar[Title].GetString(sTitle, sizeof(sTitle));
	...
	Cvar[FooterText].GetString(sFooterText, sizeof(sFooterText));
	if (!sFooterText[0])
	{
	...
	char sIcon[256];
	Cvar[Icon].GetString(sIcon, sizeof(sIcon));
	if (sIcon[0])
	{
